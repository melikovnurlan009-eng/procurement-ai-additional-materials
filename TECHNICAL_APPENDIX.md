...
|---|---|---|---|---|---|---|
| 1 | Acquire legislation | `code/scrapers/legislation/group_a_legislation_scraper_v4.py` (+ historical `_v1`/`_v2`, + later patch `scrape_missing_legislation.py`) | legislation.gov.uk XML (AKN/CLML) | `nodes_*.jsonl`, `references_*.jsonl`, `annotations_*.jsonl`, `legal_effects_*.jsonl` under an output dir | none (no LLM) | `python group_a_legislation_scraper_v4.py <args -- see its own --help>` |
