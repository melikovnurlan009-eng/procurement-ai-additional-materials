...
|---|---|---|---|---|---|
| 1 | Acquire legislation | `code/scrapers/legislation/group_a_legislation_scraper_v4.py` (run this one -- the latest version; `_v1.py`/`_v2.py` are kept for reference/audit only). Needs `requests` and `lxml` (not in `code/requirements.txt` -- install separately). Run from `code/`, since `--output-dir` is relative to it, not to `scrapers/legislation/` | legislation.gov.uk XML (AKN/CLML) | `processed/nodes.jsonl`, `references_*.jsonl`, `annotations_*.jsonl`, `legal_effects_*.jsonl` | `cd code && python scrapers/legislation/group_a_legislation_scraper_v4.py --source PA2023 --output-dir data/group_a_legislation_v4` |
| 1b | Acquire additional cited instruments (backfill, later in the pipeline -- not right after step 1) | `scrape_missing_legislation.py` -- see note below; **its own input generator is not included in this bundle (item 7, `provenance/missing_artifacts.md`)** | `evaluation/acquisition/missing_references.jsonl` (you must supply this yourself) | same outputs as step 1, for the acquired instruments | `python scrapers/legislation/scrape_missing_legislation.py --apply` (only once you have the input file) |
...
`scrape_missing_legislation.py` (step 1b) is not a simple follow-up to step 1. It backfills
instruments the corpus cites but does not hold, ranked by citation frequency -- but it can only
do that once the corpus already exists and has been through reference resolution (step 8), since
that is what identifies which citations point outside the corpus in the first place. Its default
input, `evaluation/acquisition/missing_references.jsonl`, is generated by a separate script,
`collect_missing_references.py`, which reads the ingested corpus's hyperlinks, AKN citation
targets, and the resolver's own `TARGET_NOT_IN_CORPUS`/`EXTERNAL_INSTRUMENT_REFERENCE` output.
Neither that generator script nor its output file is included in this bundle -- see item 7 of
`provenance/missing_artifacts.md`. The instruments this step already acquired are ingested into
the live, evaluated corpus as normal; only the backfill step itself cannot currently be re-run
end-to-end from the checked-out repository.
