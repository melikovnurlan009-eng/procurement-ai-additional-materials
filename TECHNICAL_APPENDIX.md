...
flowchart TD
    subgraph ACQ["1. Acquisition (code/scrapers/)"]
        A1[legislation.gov.uk XML] --> A2[group_a_legislation_scraper_v2.py or _v4.py -- both work, see note below]
...
|---|---|---|---|---|---|---|
| 1 | Acquire legislation | `code/scrapers/legislation/group_a_legislation_scraper_v2.py` **or** `_v4.py` (both verified compatible with step 5a as of the 2026-09-09 fix -- see note below); `_v1.py` also works; `scrape_missing_legislation.py` is a later supplementary patch for additional cited instruments | legislation.gov.uk XML (AKN/CLML) | `processed/nodes.jsonl`, `references_*.jsonl`, `annotations_*.jsonl`, `legal_effects_*.jsonl` under an output dir | none (no LLM) | `python group_a_legislation_scraper_v2.py --source PA2023 --output-dir data/group_a_legislation_v2` (or `_v4.py` with `--output-dir`) |
...
### 0.2a Note: `_v4.py` and `chunk_legislation_from_nodes.py` are now compatible (fixed 2026-09-09)

**Background, established by direct testing.** Three versions of the legislation scraper
coexist in this repository (`_v1.py`, `_v2.py`, `_v4.py`). `_v4.py` is explicitly documented in
its own docstring as "intentionally based on the successful V2 design principles" -- i.e. a
deliberate successor to `_v2.py` for the same three core instruments, not a separate tool for a
different purpose. It has genuine improvements over `_v2.py` (per-instrument `corpus_role`
tagging, an added `expected_min_nodes` integrity check, more granular validation), but its
rewrite also renamed two output fields (`element_type`->`node_type`, `eId`->`eid`) without
updating `chunk_legislation_from_nodes.py` to match, and left a small number of nodes (46 of
2,822 for PA2023) with a null `eId`/`eid` that the chunker did not guard against. Feeding
`_v4.py`'s real, live output directly into the unmodified chunker crashed on both counts.
Separately, three real downstream scripts (`extract_guidance_references.py`,
`resolve_references.py`, `validate_search_corpus.py`) hardcode their input path to
`data/group_a_legislation_v4/`, and `scrape_missing_legislation.py`'s docstring describes
reusing `_v4.py`'s extraction "exactly as the Procurement Act was" processed -- real evidence
that `_v4.py` is genuinely part of the pipeline, in tension with the chunker incompatibility.
Rather than leave that tension unresolved, `chunk_legislation_from_nodes.py` has now been fixed
to accept either scraper's field names.

**The fix**: `chunk_legislation_from_nodes.py::_normalize_node()` accepts either
`element_type`/`eId` (the `_v1.py`/`_v2.py` naming) or `node_type`/`eid` (the `_v4.py` naming),
and falls back to the always-present `node_id` when `eId`/`eid` is null. No other logic in the
script changed. Re-tested against `_v4.py`'s own real, unmodified, live-scraped output for
PA2023 (the exact same file that crashed before the fix): now produces 355 chunks with no
errors, and the specific chunk checked (`UKPGA_2023_54__NODEV1__CH_00051`, PA2023 s.51) is
byte-for-byte identical to the actual corpus. The full `prw` test suite was re-run afterward
(59 passed, 1 environment-only skip for a missing `scipy` install, unrelated to this change) to
confirm nothing else was affected. `_v2.py`'s output continues to work unchanged, since its
field names already match the first branch of the fallback.

With this fix, either `_v1.py`, `_v2.py`, or `_v4.py` can be used to acquire these three core
instruments and feed cleanly into `chunk_legislation_from_nodes.py`. The historical question of
which script actually produced the original corpus is no longer practically important, since
all three are now confirmed to produce an equivalent, working result for this step.
