...
| 15b | Standalone benchmark: metrics + diagnostics | `compute_metrics.py`, `strict_target_recall.py`, `candidate_ceiling.py` / `candidate_ceiling_CORRECTED.py`, `error_analysis.py` | judgments + `gold_evidence.jsonl` (see `provenance/missing_artifacts.md` item 7 for a known, unfixed citation-resolution bug affecting some of `gold_evidence.jsonl`'s regulation-style targets) | metrics JSON/CSV | see each script's `--help` |
